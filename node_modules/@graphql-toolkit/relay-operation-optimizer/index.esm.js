...
function optimizeDocuments(schema, documents, options = {}) {
    ...
    if (options.includeFragments) {
        const fragmentCompilerContext = new CompilerContext(adjustedSchema).addAll(relayDocuments).applyTransforms([transform$1, transformWithOptions({ flattenAbstractTypes: false }), transform$2]);
    ...
    const queryCompilerContext = new CompilerContext(adjustedSchema)
        ...
        .applyTransforms([transform$1, transform$3, transformWithOptions({ flattenAbstractTypes: false }), transform$2]);
